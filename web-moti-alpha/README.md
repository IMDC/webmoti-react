This is the READMe file for the Alpha version of web modi.

NB: The folder structure of this project is mostly helper functions and build functions to allow applicaton to run through zoom client.

The primary work directory(where the frontend of the application is built) is in the routes folder. As this is a single page web application, there is only one route/page, the index.html

All the backend code for communicating with the board(arduino/esp32) will be inside of the backend folder 

To setup the backend, you must upload the th .sql file and php files to a web server(such as hostzinger)

the instructions for setting up and running application in the zoom client can be found below:


NB: due to Zooms CSP most of the functionality carried out by external files such as imported js,jsx and css files fails to execute, so a way aroudn this must be found to procede, however due to limited documentation i foudn researching the topic difficult. This has led to most of the styling and script for this project to be internal.

NB: to communitcate wtih a microcontroller, upload the code from the "microcontroller" file to a microcontroller after the backend files have been uploaded to a webserver, then you  must configure the ip address, URl, passcode, etc.

to  run these files you will have to follow 
## Prerequisites
